                 ...
                 public Tracker
{
	...
	void showField( pvd::FieldConstPtr & pField, const std::string & parentName ) FINAL
	{
		if ( pField == NULL )
			return;
		...
		case pvd::unionArray:
		default:
			printf( ", TODO: Add support for pvd::Type %s\n", pvd::TypeFunc::name( pField->getType() ) );
			break;
		case pvd::scalar:
			{
			pvd::ScalarConstPtr	pScalar	= std::tr1::static_pointer_cast<const pvd::Scalar>( pField );
			if ( pScalar == NULL )
				printf( ", Error, unable to get pScalar!\n" );
			else
			{
				std::string		fullName( parentName );
				printf( "	Field: ID %10s, Name %s, Type %1d (%s)\n", pField->getID().c_str(),
						fullName.c_str(), pField->getType(), pvd::TypeFunc::name( pField->getType() ) );
			}
			}
			break;
		case pvd::structure:
			pvd::StructureConstPtr	pStruct	= std::tr1::static_pointer_cast<const pvd::Structure>( pField );
			if ( pStruct == NULL )
				printf( ", Error, unable to get pStruct!\n" );
			else
			{
				printf( ", %zu fields\n", pStruct->getNumberFields() );
    ...
    virtual void infoDone(const pvac::InfoEvent& evt) OVERRIDE FINAL
    {
        ...
        case pvac::InfoEvent::Success: {
			...
			pvd::FieldConstPtr	pInfoEvent	= evt.type;
			printf( "Top Field: ID %s, Type %d (%s)", pInfoEvent->getID().c_str(), pInfoEvent->getType(),
							pvd::TypeFunc::name( pInfoEvent->getType() ) );
			showField( pInfoEvent, op.name() );
			break;
        	}
        }
        done();
